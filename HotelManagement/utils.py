...
from sqlalchemy import text, extract, func, join, and_
...
def month_stats(from_date=None, to_date=None, kws=None):
    s = db.session.query(RoomType.id, RoomType.room_type_name,
                         func.sum(Bill.unit_price * Surchange.surchange + Bill.unit_price),
                         func.count(RentalVoucher.bill_id)) \
        .join(Bill, RentalVoucher.bill_id.__eq__(Bill.id)) \
        .join(Room, RentalVoucher.room_id.__eq__(Room.id)) \
        .outerjoin(RoomType, Room.room_type_id.__eq__(RoomType.id), isouter=True) \
        .join(Surchange, Bill.surchage_id.__eq__(Surchange.id)) \
    ...
    if kws:
        s = s.filter(RoomType.room_type_name.contains(kws))

    return s.all()


def turnover_stats(year, from_month=None, to_month=None):
    t = db.session.query(extract('month', RentalVoucher.check_in_date),
                         func.sum(Bill.unit_price * Surchange.surchange + Bill.unit_price),
                         func.count(Bill.id)) \
        .join(Bill, RentalVoucher.bill_id.__eq__(Bill.id)) \
        .join(Room, RentalVoucher.room_id.__eq__(Room.id)) \
        ...
        .order_by(extract('month', RentalVoucher.check_in_date))

    if from_month and to_month:
        t = t.filter(and_(extract('month', RentalVoucher.check_in_date) >= from_month,
                          extract('month', RentalVoucher.check_in_date) <= to_month))

    return t.all()


def count_stats(years, kw=None):
    i = db.session.query(extract('month', RentalVoucher.check_in_date), Room.room_name,
        ...
        .filter(extract('year', RentalVoucher.check_in_date) == years) \
